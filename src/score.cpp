...
char* condition_name( char_data* ch )
{
  ...
  IS_AFFECTED( ch, AFF_HALLUCINATE ) ? " hallucinating" : "");
...
void do_score( char_data* ch, char* )
{
  ...
  sprintf( buf+5,  "    Items: %d/%d       ", ch->contents.number, ch->can_carry_n( ) );
  sprintf( buf+25, "   Weight: %d/%d       ", (ch->Empty_Capacity()-ch->Capacity( ))/100, ch->Empty_Capacity( )/100 );  // ch->contents.weight/100
  ...
  page( ch, "     Hit Regen: %-8.1f Ene Regen: %-9.1f Mv Regen: %.1f\r\n",
    is_set( ch->affected_by, AFF_POISON ) ? -2 : float( ch->Hit_Regen( )/10. ), float( ch->Mana_Regen( )/10. ),
    float( ch->Move_Regen( )/10. ) );

  wield = ch->Wearing( WEAR_HELD_R, LAYER_BASE );
  wield2 = ch->Wearing( WEAR_HELD_L, LAYER_BASE );

  if( wield2  == NULL || wield2->pIndexData->item_type != ITEM_WEAPON ) {
    page( ch, "       Hitroll: %-+10.2f Damroll: %+.2f\r\n\r\n", 
      ch->Hitroll_No_Improves( wield ), ch->Damroll_No_Improves( wield ) );
    }
  else {
    page( ch,
      "     Hitroll: %+3.2f/%+3.2f      Damroll: %+3.2f/%+3.2f\r\n\r\n", 
      ch->Hitroll_No_Improves( wield ), ch->Hitroll_No_Improves( wield2 ),
      ch->Damroll_No_Improves( wield ), ch->Damroll_No_Improves( wield2 ) );
    }
  ...
  page( ch, "   %s%s%s%s\r\n\r\n", tmp+160, tmp+200, tmp+240, tmp+280 );
  ...
  float percentage = 100*total_pracs( ch )/expected_pracs( ch );
  page_centered( ch, "You have learned %s of what you can.\r\n",
    percentage < 40 ? "little" : percentage < 60 ? "some" : percentage < 80 ?
    "a considerable amount" : percentage < 100 ? "a lot" : "all" );
...
void do_statistics( char_data* ch, char* argument )
{
  ...
  if( *argument != '\0' && ch->shdata->level >= LEVEL_APPRENTICE ) {
    if( ( victim = one_player( ch, argument, "stat", (thing_array*) &player_list ) ) == NULL )
      return;
    }
  ...
  page( ch, "%13s:%7d%13s:%7d%13s:%7d\r\n",
    ...
    "+Dex", victim->shdata->level*(victim->Dexterity( )-12)/10 );
