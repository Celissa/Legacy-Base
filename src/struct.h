...
typedef class   Tprog_Data         tprog_data;
...
typedef class   Template_Data      template_data;
...
typedef class   Variable_Data      variable_data;

typedef class   Event_Data         event_data;
typedef class   Money_Data         money_data;
typedef class   Synergy            synergy;
...
typedef void         do_func     ( char_data*, char* argument );
typedef bool         spell_func  ( char_data*, char_data*, void*, int, int );
typedef void         path_func   ( char_data*, char_data*, char*, int, int );  // last two parameters defaulted to 0
typedef bool         set_func    ( char_data*, const char*, const char* );
typedef thing_data*  thing_func  ( thing_data*, char_data*, thing_data* );  // last parameter defaulted to NULL
...
#include "weather.h" // mostly included here for its timeval crap
...
typedef class Array<Clan_Data*>      clan_array;
typedef class Array<Title_Data*>     title_array;
typedef class Array<Skill_Type*>     skill_array;
typedef class Array<Rtable_Data*>    rtable_array;
typedef class llist<char>            filelist;
typedef class llist<money_data>      coin_array;
typedef class llist<Link_Data>       link_array;
typedef class llist<Variable_Data>   variable_array;

/*
 *    MISC NAUGHTY BITS WHICH SHOULD BE PUT ELSEWHERE
 */

extern int str_cmp(const char *str1, const char *str2);
filelist *dirlist(const char *filespec);

extern coin_array coinage;
void update_coinage();
...
#define OBJ_FAQ                   57
#define OBJ_GOOD_BERRY          4340
#define OBJ_BRAMBERRY            243
#define OBJ_BLESSED_FRUIT       4341
#define OBJ_RED_GREEN_FRUIT      588
#define OBJ_BLESSED_MEAL        4342
#define OBJ_ACORN                672
#define OBJ_ACORN_NECKLACE      4418
#define OBJ_SHORT               3000 
#define OBJ_SOUL_STONE          4505
...
extern const char*  weapon_attack_name [ WEAPON_COUNT ];
...
do_func do_yell;
do_func do_zap;
do_func do_blackjack;
do_func do_hamstring;
do_func do_create_poison;
do_func do_apply;
do_func do_dance;
do_func do_debug;
do_func do_sing;
...
do_func do_concoct;
do_func do_tnofind;
do_func do_rflag_find;
do_func do_investigate;
do_func do_qnedit;
do_func do_qnset;
do_func do_qndesc;
do_func do_qnreward;
...
/* DME Why in the world are these functions prototyped?  They're
   DME standard functions defined somewhere or other in various
   DME standard include files.


   DME This definition of random conflicts with the one in stdlib.h
long    random       ( void );
...
bool   remove_coins       ( char_data*, int, const char* ); 
bool   room_is_dark       ( room_data *room );
bool   is_fighting        ( char_data*, char_data* );
bool   can_switch         ( char_data* );
...
CD     *rand_tank         ( room_data*, char_data* );
CD     *random_pers       ( room_data* );
CD     *get_char_group    ( char_data*, char* );
CD     *random_enemy      ( room_data*, char_data* );
...
void   process_ic_tell    ( char_data*, char_data*, char*, char* );
...
void   send_to_area       ( const char*, area_data*, int = VIS_ALL );
...
void   backup             ( player_data*, const char* );
