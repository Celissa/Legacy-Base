...
void do_probe( char_data *ch, char *argument )
{
  ...
  if( ch->pcdata == NULL || !ch->get_skill( SKILL_PROBE ) ) {
...
void do_sweep( char_data* ch, char* )
{
  ...
  if( ch->pcdata == NULL || ch->get_skill( SKILL_SWEEP ) == UNLEARNT ) {
...
bool spell_mind_blade( char_data* ch, char_data* victim, void*, int level, int )
{
  damage_mind( victim, ch, spell_damage( SPELL_MIND_BLADE, level ),"*The mind blade" );
