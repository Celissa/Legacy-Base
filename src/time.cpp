...
void sprintf_datetime( char* tmp, const time_t time_arg )
{
  if( time_arg == 0 )
    sprintf( tmp, "%-17s", "unknown" );
  else {
    struct tm* tmPtr = localtime( &time_arg );
    sprintf( tmp, "%02d %3s %04d %02d:%02d", tmPtr->tm_mday, SMonth[ tmPtr->tm_mon ], 1900 + tmPtr->tm_year, tmPtr->tm_hour, tmPtr->tm_min );
  }
}


char* ltime( const time_t& time_arg )
{
  struct tm*  tmPtr  = localtime( &time_arg );
  char*         tmp  = static_string( );

  if( abs(int(time_arg - time(0))) > 300*24*60*60 ) 
    sprintf( tmp, "%s %s %02d %04d",
      SWeekday[tmPtr->tm_wday],
      SMonth[tmPtr->tm_mon],
      tmPtr->tm_mday, tmPtr->tm_year+1900 );
  else
...
void sprintf_date( char* tmp, const time_t time_arg )
{
  struct tm* tmPtr = localtime( &time_arg );
...
void do_lag( char_data* ch, char* argument )
{
  ...
  if( is_set( &flags, 0 ) ) {
    for( sum = 0, i = 0; i < 10; i++ )
      sum += time_history[i];
    send_underlined( ch, "Sec. of Delay         # of Cycles       Percent\r\n" );
    for( j = 0, i = 0; i < 10; i++, j = ( j == 0 ? 1 : 2*j ) ) {
      sprintf( tmp, "%.1f", (float) j/10 );
      sprintf( tmp+10, "%8s %21d %16.2f\r\n", i == 0 ? "none" : ( i == 9 ? "more" : tmp ), time_history[i], (float) 100*time_history[i]/sum );
      send( tmp+10, ch );
    }
    return;
  }

  page_underlined( ch, "Command          Calls    Average    Max Time     Total\r\n" );
...
void display_lag( char_data* ch, int i )
{
  ...
  sprintf( tmp, "%-12s %8d %11.3f %11.3f %9.1f\r\n", command_table[i].name, calls, calls == 0 ? 0 : (float) total/(1000*calls), (float) max/1000, (float) total/1000 );
  page( ch, tmp );
...
void display_time( char_data* ch, const char* text, int num, char* c2 )
{
  ...
  i = (float) 100.*total_time[num].hundred( )/total_time[ TIME_ACTIVE ].hundred( );
  j = (float) 100.*critical_time[num].hundred( )/critical_time[ TIME_ACTIVE ].hundred( );
...
void do_system( char_data* ch, char* )
{
  ...
  time_t ttime = time(0);
  strncpy(tmp, ctime(&ttime), 24);
  tmp[24] = '\0';
  send( ch, "System Time: %s\r\n", tmp);

  strncpy(tmp, ctime(&boot_time), 24);
  tmp[24] = '\0';
  send( ch, " Started at: %s\r\n\r\n", tmp);

  send_underlined( ch, "Function            Percent      Critical\r\n" );

  i = total_time[ TIME_ACTIVE ].hundred( )+total_time[ TIME_WAITING ].hundred( );
  ...
  sprintf( tmp, "Megs Written: %0.2f", ((double) bytes_written) / 1048576 );
  display_time( ch, "Update Handler",  TIME_UPDATE, tmp );

  sprintf( tmp, "Gigs Written: %d", gigs_written );
  display_time( ch, "  Regeneration",  TIME_REGEN, tmp );
...
void do_time( char_data* ch, char* argument )
{
  ...
  if( flags == 0 ) {
    send( ch, "It is %s, %d %s %d, %d:%02d.\r\n", game_day_name[ week_day() ], weather.day, game_month_name[ weather.month ], 313 + weather.year, weather.hour, weather.minute );
    return;
  }
  ...
  class type_field zonetype = { "timezone", 24, &timezones[0], &timezones[1], &pc->timezone };
...
int time_arg( char*& argument, char_data* ch, bool allow_forever, bool allow_now )
{
  int i;

  if( allow_forever && !strcasecmp( argument, "forever" ) || allow_now && !strcasecmp( argument, "now" ) ) 
    return 0;
  ...
  send( ch, "Unknown unit of time.\r\nKnown units are seconds, minutes, hours, days, and years.\r\n" );
