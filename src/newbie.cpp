...
int   find_start_entry   ( char_data*, bool );
...
{
  send( link, "\r\nWhat you entered was unintelligible.  Please press\
 return and then select\r\none of the choices given between the left\
 and right brackets.\r\n" );
  press_return( link );
...
int find_start_entry( char_data* ch, bool race )
{
  int i = 0;
  const char* table;
  ...
  if( race ) {
    const char* prace = plyr_race_table[ch->shdata->race].name;
    for( i = 0; i < MAX_ENTRY_STARTING; i++ ) {
        table = starting_table[i].name;
        if( exact_match( prace, table ) )
          break;
    }
  }
  else {
    const char* cclass = clss_table[ch->pcdata->clss].name;
    for( i = 0; i < MAX_ENTRY_STARTING; i++ ) {
        table = starting_table[i].name;
        if( exact_match( cclass, table ) )
          break;
    }
  }

  if( i == MAX_ENTRY_STARTING )
    return 0;
...
bool check_parse_name( link_data* link, char* name )
{
  ...
  for( letter = name; *letter != '\0'; letter++ ) {
    if( isdigit( *letter ) ) {
      send( link, "\r\nNames are not allowed to contain numbers.\r\n" );
      return FALSE;
    }
    
    if( ispunct( *letter ) && *letter != '-' && *letter != '\'' ) {
      send( link, "\r\nNames are not allowed to contain punctuation other than dashes and\r\napostrophes.\r\n" );
      return FALSE;
    }

    if( isupper( *letter ) && ++capitals > 3 ) {
      send( link, "\r\nNames can contain at most 3 capital letters.\r\n" );
      return FALSE;
    } 
  }

  if( length < 3 || length > 12 ) {
    send( link, "Names must be greater than 2 and less than 13 letters.\r\n" );
    return FALSE;
  }
  ...
  for( int i = 0; i < MAX_SPECIES; i++ ) {
    ...
    if( is_name( name, species->descr->name ) ) {
      send( link, "\r\nThat is the name of monster in the game and thus unallowed.\r\n" );
      return FALSE;
      }
    }
...
void nanny_new_password( link_data* link, char* argument )
{
  ...
  // Passwords may not contain tilde '~', space ' ', tab '\t', single-quote
  // ('\''), or double-quote ('\"') characters
  // Anything else is permitted for now ...
...
void nanny_show_rules( link_data* link, char* )
{
  ...
  help_link( link, "Gamedesc_2" ); //  do_help( ch, "GAMEDESC_2" );
...
void nanny_help_sex( link_data* link, char* )
{
  ...
  help_link( link, "login_sex"); //  do_help( ch, "login_sex" );
...
void nanny_help_class( link_data* link, char* )
{
  ...
  help_link( link, "login_classes" ); //  do_help( ch, "login_classes" );
...
void nanny_class( link_data* link, char* argument )
{
  ...
  for( i = 0; i < MAX_CLSS; i++ ) 
    if( fmatches( argument, clss_table[i].name ) ||
      fmatches( argument, clss_table[i].abbrev ) )
      break;

  if( !clss_table[i].open ) {
    clear_screen( ch );
    help_link( link, "class_unavailable" ); 
    press_return( link );
    return;
  }
  if( help && i != MAX_CLSS ) {
  ...
  if( clss_table[i].nc_start )
    ch->Set_Integer( PORTAL_NEW_CONT, 1 );
...
void nanny_help_race( link_data* link, char* )
{
  ...
  help_link( link, "login_races" );  //  do_help( ch, "login_races" );
  strcpy( tmp, "\r\nSelect a race [" );
...
void nanny_race( link_data* link, char* argument )
{
  ...
  for( i = 0; i < MAX_PLYR_RACE; i++ ) {
    if( plyr_race_table[i].open && allowed_race( ch, i )
  ...
  ch->shdata->race = i;
  ch->pcdata->speaking = LANG_COMMON; // LANG_HUMANIC+i;
  for( int j = 0; j < LANG_COUNT; j++ ) {
    if( is_set( &plyr_race_table[i].language, j ) )
      ch->pcdata->skill[LANG_COMMON+j] = 10;
  }
  //ch->pcdata->skill[LANG_HUMANIC+i] = 10;
  ch->pcdata->skill[LANG_COMMON] = 10;

  if( plyr_race_table[i].nc_start )
    ch->Set_Integer( PORTAL_NEW_CONT, 1 );
...
void nanny_stats( link_data* link, char* argument )
{
  ...
  if( strlen( argument ) < 2 || ( strlen( argument ) < 3 
    && isdigit( argument[1] ) ) ) {
    help_link( link, "login_stats" ); // do_help( ch, "login_stats" );
    send( ch, "\r\n" );
  } else {
    ...
    if( i != 5 ) {
      ...
      case '+' :
        if( *stat[i]+number > 18+bonus[ i ] )
          send( link, "\r\nYou can not raise a stat over its racial maximum.\r\n" );
        else
          *stat[i] += number;
        break;
      case '-' :
        if( *stat[i]-number < 3 )
           send( link, 
            "\r\nYou can not lower a stat below 3.\r\n" );
        else
          *stat[i] -= number;
        break;
      default:
  ...
  send( link, "Ability             Character    Racial Max    Cost to inc.\r\n" );
  send( link, "-------             ---------    ----------    ------------\r\n" );
...
void nanny_help_alignment( link_data* link, char* )
{
  ...
  help_link( link, "login_alignment" ); //  do_help( ch, "login_alignment" );
  strcpy( tmp, "\r\nEnter an alignment [" );
...
void nanny_alignment( link_data* link, char* argument )
{
  ...
  send( link, "\r\nA%c %s would not choose to be %s.\r\n",
...
void new_player( player_data* ch )
{
  ...
  set_bit( ch->pcdata->pfile->flags, PLR_SAY_REP_COLOR );
  ...
  set_bit( ch->pcdata->pfile->flags, PLR_AUTO_ASSIST );
  set_bit( ch->pcdata->pfile->flags, PLR_CHANT );
 
  set_level( &ch->pcdata->pfile->settings, SET_AUTOLOOT, 3 );
  set_level( &ch->pcdata->pfile->settings, SET_AUTOSCAN, 3 );
  set_level( &ch->pcdata->pfile->settings, SET_IGNORE, 0 );
  set_level( &ch->pcdata->pfile->settings, SET_INCOGNITO, 0 );
  set_level( &ch->pcdata->pfile->settings, SET_ROOM_INFO, 1 );
...
//  set_level( ch->iflag, MAX_IFLAG+NOTE_GENERAL, 1 );
  ...
  set_level( &ch->pcdata->mess_settings, MSG_EQUIP, 1 );
  set_level( &ch->pcdata->mess_settings, MSG_INVENTORY, 1 );
  set_level( &ch->pcdata->mess_settings, MSG_BANK, 1 );
  set_level( &ch->pcdata->mess_settings, MSG_PREPARE, 1 );
  ...
  sprintf( tmp, "%s %s", ch->sex == SEX_MALE ? "male" : "female", 
  ...
  sprintf( tmp, " the %s %s", ch->sex == SEX_MALE ? "male" : "female",
    race_table[ ch->shdata->race ].name );
  ...
  use_start_table( ch, find_start_entry( ch, FALSE ) );
  use_start_table( ch, find_start_entry( ch, TRUE ) );
...
void use_start_table( char_data* ch, int entry )
{
  ...
  for( int i = 0; i < 5; i++ ) 
    if( ( skill = starting_table[entry].skill[i] ) >= 0 )
      ch->pcdata->skill[skill] = starting_table[entry].level[i];

  for( int i = 0; i < 5; i++ ) 
    if( ( index = get_obj_index( starting_table[entry].object[2*i] ) ) != NULL ) {
...
{
  if( ch->species != NULL || ch->played + time(0) - ch->logon > 50 )
    return FALSE;

  send( ch, "To stop abuse you cannot drop or give anything for the first 5 minutes\r\nof play.  To get rid of an item use junk.\r\n" );
