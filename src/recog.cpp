...
bool display( char_data* ch, int bit, const char* title )
{
  ...
  for( i = j = 0; i < recognize->size; i++ ) {
    if( ( pfile = get_pfile( recognize->list[i] & 0xffff ) ) == NULL || !is_set( &recognize->list[i], bit ) )
      continue;
    if( !found ) { 
      page( ch, "%s:\r\n", title );
      found = TRUE;
...
pfile_data* remove_name( recognize_data*& recognize, char* name, int bit )
{
  ...
  if( ( pfile = find_pfile( name ) ) != NULL && remove_recognize_bit( recognize, pfile->ident, bit ) )
    return pfile;
...
{
  if( pcdata == NULL || victim == NULL )
    return FALSE;

  if( victim->species != NULL ) {
    if( victim->descr == victim->species->descr || victim->leader != this || victim->descr->singular[0] == '\0' )
      return FALSE;
    return TRUE;
  }

  if( victim == this || shdata->level >= LEVEL_APPRENTICE || victim->shdata->level >= LEVEL_APPRENTICE ) 
    return TRUE;

  if( victim->pcdata != NULL && this->pcdata != NULL && this->pcdata == victim->pcdata )
    return TRUE;

  return is_recognize_set( pcdata->recognize, victim->pcdata->pfile->ident, RECOG_RECOGNIZE );
}


bool char_data :: Recognizes( pfile_data* pfile )
{
  if( pcdata == NULL || pfile == NULL )
    return FALSE;

  if( pcdata->pfile == pfile )
    return TRUE;

  if( shdata->level >= LEVEL_APPRENTICE || pfile->level >= LEVEL_APPRENTICE )
    return TRUE;

  return is_recognize_set( pcdata->recognize, pfile->ident, RECOG_RECOGNIZE );
}
...
void do_introduce( char_data *ch, char *argument )
{
  ...
  if( ( victim = one_character( ch, argument, "introduce", ch->array ) ) == NULL )
    return;
  ...
  if( victim->fighting != NULL ) {
    send( ch, "They are too busy fighting for you to introduce yourself.\r\n" );
    return;
  }
...
bool is_befriended( pfile_data *pfile, char_data *ch )
{
  player_data*   player;
  ...
  for( link_data* link = links.first(); link != NULL; link = links.next() ) {
    if( ( player = link->player ) != NULL && player->pcdata->pfile == pfile ) {
      // can test with connected players
      return player->Befriended( ch );
    }
  }
  ...
  if( !load_char( link, pfile->name, PLAYER_DIR ) ) {
    bug( "Is_Befriended: Non-existent player file (%s) requested by %s", pfile->name, ch->real_name( ) );
    delete link;
    return false;
  }
  ...
  bool befriended = player->Befriended( ch );
...
bool Char_Data :: Befriended( pfile_data* pfile )
{
  ...
  return is_recognize_set( pcdata->recognize, pfile->ident, RECOG_BEFRIEND );
}
...
bool Char_Data :: Befriended( char_data* ch )
{
  ...
  return is_recognize_set( pcdata->recognize, ch->pcdata->pfile->ident, RECOG_BEFRIEND );
}
...
void do_befriend( char_data* ch, char* argument )
{
  ...
  if( ( pfile = remove_name( ch->pcdata->recognize, argument, RECOG_BEFRIEND ) ) != NULL ) {
    sprintf( tmp, "You no longer consider %s a friend.\r\n", pfile->name );
    send( tmp, ch );
    return; 
  }
        
  if( ( victim = one_character( ch, argument, "befriend", ch->array ) ) == NULL )
    return;
  ...
  if( !ch->Recognizes( victim ) ) {
    fsend( ch, "You cannot befriend %s without first knowing %s name.", victim, victim->His_Her( ) );
    return;
  }

  if( !victim->Recognizes( ch ) ) {
    fsend( ch, "You cannot befriend %s until you have introduced yourself to %s.", victim, victim->Him_Her( ) );
    return;
  }
...
{
  if( pcdata == NULL || victim->pcdata == NULL || victim->shdata->level >= LEVEL_APPRENTICE )
    return FALSE;

  if( !is_set( pcdata->pfile->flags, PLR_APPROVED ) && has_permission( victim, PERM_APPROVE ) )
    return FALSE;
...
void do_filter( char_data* ch, char* argument )
{
  ...
  if( ( victim = one_player( ch, argument, "filter", (thing_array*) &player_list ) ) == NULL ) 
    return;
...
{
  if( ch == NULL || ch == victim || ch->shdata->level >= LEVEL_APPRENTICE 
    || ch->pcdata == NULL )
    return TRUE;

  if( ch->pcdata != NULL && victim->pcdata != NULL && is_recognize_set( victim->pcdata->recognize, ch->pcdata->pfile->ident, RECOG_CONSENT ) )
    return TRUE;

  if( ch->pcdata != NULL && victim->pcdata == NULL && victim->leader == ch && is_set( &victim->status, STAT_PET ) )
    return TRUE;
...
void do_consent( char_data* ch, char* argument )
{
  ...
  if( ( victim = one_player( ch, argument, "consent", ch->array, (thing_array*) &player_list ) ) == NULL )
    return;
