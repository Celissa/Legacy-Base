...
void add_log( const char* function, const char* file, char_data* ch, const char* string )
{
//  char      tmp  [ MAX_INPUT_LENGTH+20 ];
  ...
  fprintf( fp, "[%s %10s] %s\r\n", ltime( time(0) ) + 4, ch->real_name( ), string );
...
void do_mexplog( char_data* ch, char* argument )
{
  char               tmp  [ ONE_LINE ];
  ...
  if( is_number( argument ) ) { 
    ...
    send( ch, "This command only works with the mobs vnum.\r\n" );
    return;
  }

  sprintf( tmp, "%s%d.log", MOB_EXP_LOG_DIR, species->vnum );
  if( !view_file( ch, tmp ) )
...
void do_mlog( char_data* ch, char* argument )
{
  ...
  if( is_number( argument ) ) { 
    ...
    if( ( pfile = find_pfile_exact( argument ) ) == NULL ) {
      if( ( victim = one_character( ch, argument, "mlog", (thing_array*) &player_list, (thing_array*) &mob_list ) ) == NULL ) 
        return;
...
void do_olog( char_data* ch, char* argument )
{
  ...
  if( *argument == '\0' ) {
    if( ( obj_clss = wizard->obj_edit ) == NULL ) {
      send( ch, "You are not editing an object so must specify a vnum.\r\n" );
      return;
    }
    i = obj_clss->vnum;
  } else {
...
void player_log( pfile_data* pfile, const char* string )
{
  ...
  if( pfile == NULL || string == empty_string )
    return;

  sprintf( file, "%s%s", PLAYER_LOG_DIR, pfile->name );

  if( ( fp = open_file( file, "ab" ) ) == NULL )
    return;

  fprintf( fp, "[%s] %s\r\n", ltime( time(0) ) + 4, string );
  fclose( fp );
...
void immortal_log( char_data* ch, char* cmd, char* arg )
{
  ...
  if( ( fp = fopen( file, "ab" ) ) != NULL ) {
    fprintf( fp, "[%s - %5d] %s %s\n",
      ltime( time(0) ) + 4,ch->in_room->vnum, cmd, arg );
    fclose( fp );
    }
