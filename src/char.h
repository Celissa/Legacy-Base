...
class Char_Data : public Thing_Data
{
 public: 
  ...
  int           original_hit;
  // these 6 values must remain grouped
  int                    hit;
  int               base_hit;
  int                   mana;
  int              base_mana;
  ...
  int              mod_damage  [ 10 ];                 // should be MAX_ATTACK
  int               mod_armor;
  ...
  // try basing experience on each individual mob
  // rather than allowing it to float with the species
  // data
  // clear these values on reset if they weren't fighting
  // recently if no players in zone..that'll work
  ...
  const char*   Deity           ( char_data* = NULL, int = REL_NONE );
  const char*   Lad_Lass        ( char_data* = NULL );
  const char*   Sir_Maam        ( char_data* = NULL );
  const char*   Boy_Girl        ( char_data* = NULL );
  const char*   Man_Woman       ( char_data* = NULL );
  ...
  float          Hitroll_No_Improves ( obj_data* );
  float          Damroll_No_Improves ( obj_data* );
  int            Hit_Regen       ( );
  int            Move_Regen      ( );
  int            Mana_Regen      ( );
  ...
  obj_data*      WearingShield   ( int, int = -1 );
  ...
  int            get_skill       ( int );
  int            real_skill      ( int );
  int            get_skill_usage ( int );
  time_t         time_played     ( );

  void          improve_skill    ( int );

  void sendf(const char *format, ...);
  void pagef(const char *format, ...);
};
...
class Player_Data : public Char_Data
{
 public:
  ...
  VARIABLE_POINTER(note_data, note_buffer);         // currently edited note buffer
  VARIABLE_POINTER(noteboard_data, current_board);  // currently selected noteboard
};
...
class Wizard_Data : public player_data
{
 public:
  ...
  TQuest*                QuestEdit;
  TQuestStep*        QuestStepEdit;
  TQuestReward*    QuestRewardEdit;
  Synergy*             SynergyEdit;
  species_data*           mob_edit;
  int                  custom_edit;
  rtable_data*         rtable_edit;
  int                    list_edit;
  int                   table_edit  [ 2 ];
...
class Share_Data
{
 public:
  ...
  int  modify_damage  [ 10 ];   // should be MAX_ATTACK
