...
void do_ban( char_data* ch, char* argument )
{
  ...
  if( is_set( &flags, 2 ) ) {
    if( *argument == '\0' ) {
      if( badname_array == NULL ) {
        ...
        display_array( ch, "Bad Name Array", &badname_array[0], &badname_array[1], max_badname );
      }
      return;
    }
  ...
  for( i = 0; i < max_pfile; i++ )
    if( pfile_list[i]->account == account && pfile_list[i]->trust >= ch->pcdata->trust ) {
  ...
  for( i = 0; i < player_list; i++ ) {
    ...
    if( victim->Is_Valid( ) && victim->pcdata->pfile->account == account ) {
...
bool is_banned( const char* host )
{
  ...
  for( ban = ban_list; ban != NULL; ban = ban->next ) { 
    ...
    if( ban_len <= host_len && !strcasecmp( &host[host_len-ban_len], ban->name ) ) 
      break;
  }
...
bool is_banned( account_data* account, link_data* link )
{
  ...
  if( account->banned != 0 && account->banned < time(0) ) {
    account->banned = -1;
    return FALSE;
  }
...
void load_badname( void )
{
  ...
  log( "Loading Bad Names...\r\n" );

  if( ( fp = fopen( BADNAME_FILE, "rb" ) ) == NULL ) {
    perror( BADNAME_FILE );
    throw( 1 );
    }
  ...
  for( ; ; ) {
    ...
    if( *name == '$' ) 
      break;
    record_new( sizeof( char* ), MEM_BADNAME );
    insert( badname_array, max_badname, (const char *) name, max_badname );
    }
...
void save_badname( void )
{
  ...
  rename_file( AREA_DIR, BADNAME_FILE, PREV_DIR, BADNAME_FILE );

  if( ( fp = fopen( BADNAME_FILE, "wb" ) ) == NULL ) {
...
void load_banned( void )
{
  ...
  log( "Loading Banned Sites...\r\n" );

  if( ( fp = fopen( BANNED_FILE, "rb" ) ) == NULL ) {
    perror( BANNED_FILE );
    throw( 1 );
    }
...
void save_banned( )
{
  ...
  rename_file( AREA_DIR, BANNED_FILE, PREV_DIR, BANNED_FILE );

  fp = open_file( BANNED_FILE, "wb" ); 
