...
{  
  ...
  if( ( fp = open_file( "email.msg", "wb" ) ) == NULL ) 
    return;

  if( change ) {
    fprintf( fp, "A request was entered at Legacy's Dawning to change\n" );
    ...
    fprintf( fp, "A request was entered at Legacy's Dawning to open an\n" );
  ...
  //Sanity check
  if( valid_email( account->new_email ) ) {  
#ifdef MYSTERIA_WINDOWS
    log("Spawned Email Handler: %d for %s", spawnl(_P_NOWAIT, "softmail.exe", account->new_email, account->new_email, NULL), account->new_email);
#else
    char tmp  [ MAX_INPUT_LENGTH ];
    sprintf( tmp, "/usr/lib/sendmail -fjmccaughey@sympatico.ca -t %s < email.msg", account->new_email );
    system( tmp );
#endif
    return;
  }
  send( link, "The e-mail address is invalid.\r\n" );
  return;
}
...
void nanny_acnt_email( link_data* link, char* argument )
{
  ...
  if( is_banned( argument ) ) {
    send( link, "New accounts with email from that site are banned.\r\n" );
...
void nanny_acnt_menu( link_data* link, char* argument )
{
  ...
  case 1:
    if( !link->hostresolved ) {
      send( link, "Your site is not yet validated, please wait a few moments then try again.\r\n" );
      write_greeting( link );
      link->connected = CON_INTRO;
      break;
    }
...
#ifdef MYSTERIA_WINDOWS
    log("Spawned Email Handler: %d for %s", spawnl(_P_NOWAIT, "softmail.exe", account->email, account->email, NULL), account->email);
//    log("Spawned Email Handler: %d for %s", spawnl(_P_NOWAIT, "softmail.exe", account->email, NULL), account->email);
#else
    char tmp  [ MAX_INPUT_LENGTH ];
    sprintf( tmp, "/usr/lib/sendmail -fjmccaughey@sympatico.ca -t %s < email.msg", account->email );
    system( tmp );
#endif
  } else {
    send( link, "The e-mail address is invalid.\r\n" );
...
void save_accounts( )
{
  ...
  rename_file( AREA_DIR, ACCOUNT_FILE, PREV_DIR, ACCOUNT_FILE );

  if( ( fp = open_file( ACCOUNT_FILE, "wb" ) ) == NULL ) 
    return;
  ...
  for( int i = 0; i < max_account; i++ ) {
    ...
    fprintf( fp, "%s~\n", account->confirm );
    fprintf( fp, "%s~\n", account->notes );
    fprintf( fp, "%d %d %d\n\n", account->last_login, account->balance, account->banned );
  }
...
void load_accounts( )
{
  ...
  if( ( fp = fopen( ACCOUNT_FILE, "rb" ) ) == NULL ) {
...
void display_pfile( char_data* ch, pfile_data* pfile, bool& first )
{
  ...
  sprintf( tmp, "%-14s%-30s%-18s%-10s%5d\r\n", pfile->name, host, ltime( pfile->last_on )+4, clss_table[pfile->clss].name, pfile->level );
  page( ch, tmp );
}
...
void do_accounts( char_data* ch, char* argument )
{
  ...
  if( !get_flags( ch, argument, &flags, "spPb", "Accounts" ) )
    return;

  // accounts -b : list players from banned accounts
  if( is_set( &flags, 3 ) ) {
    for( i = 0; i < max_pfile; i++ ) {
      ...
      if( account->banned != -1 && imm->See_Account( pfile ) ) {
        if( first ) {
          page( ch, "%-14s %-s\r\n", "Player", "Banned Until" );
          first = FALSE;
        }
        
        page( ch, "%-14s %-s\r\n", pfile->name, account->banned == 0 ? "forever" : ltime( account->banned ) );
      }
    }
    
    if( first )
      send( ch, "No players are banned.\r\n" );
    
    return;
  }

  // accounts -s : list players by site
  if( is_set( &flags, 0 ) ) {
    if( *argument == '\0' ) {
  ...
  // accounts -P : list pfiles using a certain password
  if( is_set( &flags, 2 ) ) {
    if( *argument == '\0' ) {
      send( ch, "For which character to you wish to run a password match?\r\n" );
      return;
    }
    ...
    for( i = 0; i < max_pfile; i++ ) 
      if( !strcasecmp( pfile_list[i]->pwd, pfile->pwd ) && imm->See_Account( pfile_list[i] ) )
        display_pfile( ch, pfile_list[i], first );
      
    return;
  }          

  // accounts : Syntax error.  Need an argument or a flag.
  if( *argument == '\0' ) {
    send( ch, "For what player %sdo you wish to list an account summary?\r\n", is_set( &flags, 2 ) ? "" : "or account " );
    return;
  }

  // From here to end of routine:
  // accounts -p <player> : list players in account containing <player>
  // accounts <account>   : list players in account <account>
  ...
  if( account->banned != -1 ) 
    page( ch, "   Banned: %s\r\n", account->banned == 0 ? "forever" : ltime( account->banned ) ); 
