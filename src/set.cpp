...
void set_string( char_data* ch, const char* argument, char*& string, const char* subject, int mem_type )
{
  if( *argument == '\0' ) {
...
const char* string_field :: set( char_data* ch, const char* subject, const char* argument )
{
...
void set_type( char_data* ch, const char* argument, int& value, const char* field, const char* subject, int max, const char** word1, const char** word2 )
{
  int i;
...
const char* type_field :: set( char_data* ch, const char* subject, const char* argument )
{
  ...
  for( i = 0; i < max; i++ ) {
    if( !strncasecmp( element( i ), argument, length ) ) {
      *value = i;
      ...
      tmp[0] = toupper( tmp[0] );
      send( tmp, ch );
      sprintf( buf, "%s set to %s. [%s]", name, element( i ), ch->Name( ) );
      return buf;
    }
  }
...
const char* int_field :: set( char_data *ch, const char* subject, const char *argument )
{
...
const char* cent_field :: set( char_data *ch, const char* subject, const char *argument )
{
  char*  tmp  = static_string( );
  int    num;
  ...
  sprintf( tmp, "%s set to %.2f from %.2f.", name, (float) num/100, (float) *value/100 );
...
const char* dice_field :: set( char_data* ch, const char* subject, const char* argument )
{
  static char    buf  [ TWO_LINES ];
  char           tmp  [ TWO_LINES ];
  dice_data    dice1;
  ...
  sprintf( tmp, "%s on %s set to %dd%d+%d.\r\n[ Prev. Value: %dd%d+%d ]\r\n", name, subject, dice2.number, dice2.side, dice2.plus, dice1.number, dice1.side, dice1.plus );
  send( tmp, ch );

  *value = (int) dice2;

  sprintf( buf, "%s set to %dd%d+%d.", name, dice2.number, dice2.side, dice2.plus );
