...
Pfile_Data :: ~Pfile_Data( )
{
  ...
  if( clan2 != NULL ) 
    remove_member( clan2, this );
  if( guild != NULL ) 
    remove_member( guild, this );

  for( int i = 0; i < MAX_IFLAG + 1; i++ )
    for( info_data* info = info_history[ i ]; info != NULL; info = info->next )
      if( info->pfile == this )
        info->pfile = NULL;
...
void extract( pfile_data* pfile, link_data* prev )
{
  ...
  for( link_data* link = links.first(); link != NULL; link = links.next() ) {
    if( link->pfile == pfile && link != prev ) {
      send( link, "\r\n\r\n+++ Character was deleted - Closing link +++\r\n" );
  ...
  for( int i = 0; i < player_list; i++ ) {
    ...
    if( victim->Is_Valid( ) && ( pos = search( victim->pcdata->recognize, pfile->ident ) ) >= 0 )
      remove( victim->pcdata->recognize, pos );
  }

  for( int i = 0; i < obj_list; i++ ) {
    if( obj_list[ i ] && obj_list[ i ]->Is_Valid() && obj_list[ i ]->owner == pfile )
...
void modify_pfile( char_data* ch )
{
  ...
  if( ident_list[pfile->ident] == NULL ) {
    ...
    if( ident_list[pfile->ident] != pfile ) {
      ...
      roach( "-- Ch = { %s, %s }", pfile->name, ident_list[pfile->ident]->name );
...
{
  for( int i = 0; i < player_list; i++ )
    if(player_list[i]->Is_Valid() && player_list[i]->pcdata->pfile == pfile && player_list[i]->in_room != NULL )
...
void forced_quit( player_data* ch, bool crash )
{
  ...
  for( int i = 0; i < ch->followers; i++ )
    if( ( rch = character( ch->followers[i] ) ) != NULL )
  ...
  if( link != NULL ) {
    ...
    reset_screen( ch );
    send( link, "Thank you for visiting Legacy's Dawning.\r\n" );
  }

  send_seen( ch, "%s has left the game.\r\n", ch );

  sprintf( tmp, "%s has quit.", ch->descr->name );
  info( "", ( imm != NULL && is_set( ch->pcdata->pfile->flags, PLR_WIZINVIS ) ? imm->wizinvis : 0 ), tmp, IFLAG_LOGINS, 1, ch );
  ...
  write( ch );
  backup( ch, PLAYER_QUIT_DIR );
  reference( ch, ch->contents, -2 );

  link_nav links( &link_list );
  for( link_data* link_new = links.first(); link_new != NULL; link_new = links.next() ) {
    if( link_new != link && link_new->character != NULL && !strcmp( link_new->character->descr->name, ch->descr->name ) ) {
      write_to_buffer( link_new, "\r\n\r\nFor security reasons closing link : please reconnect.\r\n" );
...
void do_quit( char_data* ch, char* )
{
  ...
  if( ch->shdata->level < LEVEL_APPRENTICE && can_pkill( ch, NULL, FALSE ) ) {
...
void do_delete( char_data* ch, char* argument )
{
  ...
  if( ch->shdata->level > 5 && ch->pcdata->trust < LEVEL_APPRENTICE ) {
    send( ch, "Characters over level five may not delete.  Time has shown that\r\n"
              "players often delete and then regret the action.  It also serves no purpose.\r\n"
              "If you no longer wish to play just quit and do not return.\r\n" );
    return;
  }

  if( strcmp( argument, ch->pcdata->pfile->pwd ) ) {
    send( ch, "You must type delete <password> to delete you character.\r\n" );
    return;
  }
  ...
  sprintf( tmp, "%s has deleted %sself.", ch->descr->name, ch->Him_Her( ) );
...
void purge( player_data* ch )
{
  ...
  delete_file( PLAYER_QUIT_DIR,    ch->descr->name, FALSE );
  delete_file( PLAYER_DEATH_DIR,   ch->descr->name, FALSE );
  delete_file( PLAYER_RESTORE_DIR, ch->descr->name, FALSE );
  delete_file( MAIL_DIR,           ch->descr->name, FALSE );

  if( ch->link != NULL && boot_stage == BOOT_COMPLETE ) {
    ch->link->player = NULL;
...
void do_title( char_data* ch, char* argument )
{
  ...
  */
  ...
  if( !is_set( ch->pcdata->pfile->flags, PLR_APPROVED ) ) {
    send(ch, "You must be approved before you can set your title.\r\n");
    return;
  }

  if( is_set( ch->pcdata->pfile->flags, PLR_NO_TITLE ) ) {
    send( ch, "You have had your title altering permissions removed.\r\n" );
    return;
    }
  ...
  sprintf( tmp, "%s%s", *argument == ',' || *argument == '\'' ? "" : " ", argument );
...
void do_password( char_data* ch, char* argument )
{
  ...
  { 
    if( *p == '~' || *p == ' ' || *p == '\t' || *p == '\"' || *p == '\'' ) 
    { 
      send( ch, "New password not acceptable, try again.\r\n" );  
      return;
    }
  }
...
void do_high( char_data* ch, char* argument )
{
  ...
  if( isalpha( *argument ) ) {
    for( clss = 0; clss < MAX_CLSS; clss++ )
      if( ( clss_table[clss].open || is_apprentice( ch ) ) && matches( argument, clss_table[clss].name ) )
        break;
    ...
    if( c1 < 0 || c1 >= max_high ) {
      send( ch, "That input is not understood the number must be between 0 and %i", max_high );
      return;
    }
  ...
  for( i = 0; i < 10; i++ ) {
    sprintf( r1,  "[%d]", c1+i );
    sprintf( r2,  "[%d]", c2+i );
    sprintf( tmp, "%12s  %-20s %5s  %s\r\n", r1, high_score_name( clss == -1 ? c1+i-1 : who[i], ch ), r2, high_score_name( clss == -1 ? c2+i-1 : who[10+i], ch ) );
    send( tmp, ch );
  }
}   
...
void update_score( char_data* ch )
{
  ...
  for( pos = pfile->rank; pos > 0; pos-- ) {
    ...
    m2swap( high_score[pos], high_score[pos-1] );
  }
...
int search( pfile_data** list, int max, pfile_data* pfile, pfcomp_func* compare )
{
  int     min  = 0;
  int     mid;
  int   value;
...
void sort_players( )
{
  ...
  for( i = 0; i < player_list.size - 1; i++ ) {
    ...
    for( j = 0; j < player_list.size - 1 - i; j++ ) {
      if( strcasecmp( player_list[j]->descr->name, player_list[j+1]->descr->name ) > 0 ) {
        m2swap( player_list.list[j], player_list.list[j+1] );
        done = FALSE;
      }
    }
    if( done )
...
player_data* link_player( char_data* ch )
{
  player_data* pc = player( ch );
  if( pc == NULL && ch->link != NULL && ch->link->player != ch )
    pc = ch->link->player;
