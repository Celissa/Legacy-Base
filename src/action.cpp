...
void  song_message( char_data* );
void  dance_message( char_data* );
void  sonic_damage( char_data* );
void  dance_damage( char_data* );

void  dance_destruction( char_data* );
void  dance_armageddon( char_data* );
void  dance_swirling( char_data* );
...
const char *action_value[ MAX_ATN_TRIGGER ] = {
  "unused",                // none
  "unused",                // entering room
  "chance",                // random
  "unused",                // leaving room
  "unused",                // random_always
  "unused",                // sacrifice
  "minutes past midnight", // time
  "unused",                // attack
  "unused",                // open_door
...
{
  { "loop_follower", "mounted", "resting", "sleeping", "fighting", "" }, // none
  { "north", "east", "south", "west", "up", "down", "" },                // entering
  { "" },                                                                // random
  { "north", "east", "south", "west", "up", "down", "" },                // leaving
  { "" },                                                                // random_always
  { "" },                                                                // sacrifice
  { "" },                                                                // time
  { "" },                                                                // attack
  { "north", "east", "south", "west", "up", "down", "" },                // open
  { "" },                                                                // searching
  { "north", "east", "south", "west", "up", "down", "" },                // close
  { "north", "east", "south", "west", "up", "down", "" },                // lock
  { "north", "east", "south", "west", "up", "down", "" },                // unlock
  { "north", "east", "south", "west", "up", "down", "" },                // knock
  { "north", "east", "south", "west", "up", "down", "" },                // untrap
  { "" },                                                                // get
  { "" },                                                                // repop
  { "here", "north", "east", "south", "west", "up", "down", "" },        // scan
  { "north", "east", "south", "west", "up", "down", "" },                // climb
  { "" },
};
...
const char** action_msgs [ MAX_ATN_TRIGGER ] =
{
  ...
  close_msg,          // close
  lock_door_msg,      // lock
  unlock_door_msg,    // unlock
  knock_door_msg,     // knock
  untrap_door_msg,    // untrap
  NULL,               // get
  NULL,               // repop
  scan_msg,           // scan
  NULL,               // rdesc
  ...
  for( action = room->action; action != NULL; action = action->next ) 
    //if( member( command, action->command, TRUE ) &&
     // action->target[0] == '*' )
      //break;
    if( member( command, action->command, TRUE )
      && ( member( argument, action->target ) || action->target[0] == '*' ) ) 
      break;
...
bool check_action_flags( char_data *ch, int flags, bool msg )
{
  ...
  int bit[types] = { AFLAG_MOUNTED, AFLAG_FIGHTING, AFLAG_RESTING, AFLAG_SLEEPING };
  ...
  for( area_data *area = alist.first( ); area; area = alist.next( ) )
    for( room_data *room = area->room_first; room != NULL; room = room->next ) 
      for( action_data *action = room->action; action != NULL; action = action->next ) 
        if( action->trigger == TRIGGER_TIME && action->value == weather.minute+60*weather.hour ) {
          var_room = room;
          execute( action );
        }
...
void do_aedit( char_data *ch, char *argument )
{
  ...
  if( *argument == '\0' ) {
    ...
    for( i = 0, action = room->action; action != NULL; action = action->next ) {
      if( *action->command != '\0' )
        sprintf( tmp, "[%2d] %-20s %s - %s\r\n", ++i, "Command", action->command, action->target );
      else {
        sprintf( tmp, "[%2d] %-20s %s\r\n", ++i, action_trigger[action->trigger], Keywords( action ) );
        tmp[5] = toupper( tmp[5] );
      }
      send( ch, tmp );
    }
  ...
  if( is_number( arg ) ) {
    i = atoi( arg );
    for( action = room->action; action != NULL && i != 1; action = action->next, i-- );
    if( action == NULL ) {
...
//  const char*    response;
  ...
  if( *argument == '\0' ) {
    display_flags( "Action", &action_flags[action->trigger][0], &action_flags[action->trigger][1], &action->flags, max, ch );
    return;
  }

  if( set_flags( &action_flags[action->trigger][0], &action_flags[action->trigger][1], &action->flags, max, NULL, ch, argument, FALSE, TRUE ) == NULL ) 
    return;
...
void do_astat( char_data* ch, char* )
{
  ...
  page( ch, "%10s : %s\r\n", "Trigger", action_trigger[ action->trigger ] );
  page( ch, "%10s : %d\r\n", "Value", action->value );
...
void do_aset( char_data* ch, char* argument )
{
  ...
  class type_field type_list[] = {
    { "trigger",  MAX_ATN_TRIGGER,  &action_trigger[0],  &action_trigger[1],  &action->trigger },
    { "",         0,                NULL,     NULL,     NULL             }
  };
...
void do_asave( char_data* ch, char* argument )
{
  ...
  wizard->code_buffer = action->code;
  send( ch, "Acode saved.\r\n" );
}
*/
...
void do_adata( char_data* ch, char *argument )
{
  ...
  if( wizard->adata_edit != NULL ) {
    ...
    wizard->adata_edit->text = edit_string( ch, argument, wizard->adata_edit->text, MEM_EXTRA );
  } else {
    if( *argument == '\0' )
      show_defaults( ch, action->trigger, action_msgs );

    edit_extra( action->data, wizard, offset( &wizard->adata_edit, wizard ), argument, "adata" );
    room->area->modified = TRUE;
  }
...
// passive funcs called via the 6 second pulse
...
void random_passive( void )
{
  ...
  for( int i = 0; i < player_list; i++ ) {
    ch = player_list[i];
    if( ch->In_Game( ) && ch->pcdata->clss == CLSS_DANCER ) {
      song_message( ch );
      dance_message( ch );
      if( ch->fighting != NULL )
        sonic_damage( ch );
...
void random_dance( void )
{
  ...
  for( int i = 0; i < player_list; i++ ) {
    ...
    if( ch->In_Game( ) ) {
      if( ch->fighting != NULL ) {
        if( is_set( &ch->dance, DANCE_H_STRIKES ) || is_set( &ch->dance, DANCE_T_STRIKES ) )
          dance_damage( ch );
        if( is_set( &ch->dance, DANCE_DESTRUCTION ) ) 
          dance_destruction( ch ); 
        if( is_set( &ch->dance, DANCE_ARMAGEDDON ) )
          dance_armageddon( ch ); 
        if( is_set( &ch->dance, DANCE_WHIRLING_SWORDS ) )
          dance_swirling( ch ); 
      }
    }
  }
