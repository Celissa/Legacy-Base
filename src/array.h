...
public:
  ...
  void add( something x ) {
    if( allow_multiple || !includes( x ) ) {
      if( size == memory ) {
        memory = 2 * memory + 1;
        record_new( ( memory - size ) * sizeof( something ), -MEM_ARRAY );
  ...
  friend void remove( Array& array, int i ) {
    array.size--;
    memcpy( &array.list[i], &array.list[i+1], (array.size-i)*sizeof( something ) );
  }

  friend void insert( Array& array, something x, int i ) {
    if( array.size == array.memory ) {
      array.memory = 2*array.memory+1;
      record_new( (array.memory-array.size)*sizeof( something ), -MEM_ARRAY );
      something* next = new something [ array.memory ];
      memcpy( next, array.list, i*sizeof( something ) );
      memcpy( &next[i+1], &array.list[i], (array.size-i)*sizeof( something ) );
      if( array.list != NULL )
        delete [] array.list;
      array.list = next;
    } else {
      memmove( &array.list[i+1], &array.list[i], (array.size-i)*sizeof( something ) );
    }
    array.list[i] = x;
    array.size++;
  }
  ...
  friend void copy( Array& to, Array& from ) {
    ...
    to.allow_multiple = from.allow_multiple;
