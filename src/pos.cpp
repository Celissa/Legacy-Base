...
const char* position_verb [] = { "dying", "dying",
  "dying", "dying", "sleeping", "meditating", "resting",
  "fighting", "standing", "flying" };
...
{
  /*
  ** There is no Obj_Data::character field.
...
void do_meditate( char_data* ch, char* )
{
  ...
  if( ch->pcdata == NULL || !ch->get_skill( SKILL_MEDITATE ) ) {
...
void do_sit( char_data* ch, char* argument )
{
  ...
  if( ch->position != POS_STANDING || ch->mount != NULL ) {
    send( ch, "You need to be standing before you can sit on an object.\r\n" );
    return;
  }

  if( ch->rider != NULL ) {
    send( ch, "You need to have no one riding you to sit on an object.\r\n" );
    return;
    }

  if( ch->pos_obj != NULL ) {
    send( "You are sitting and standing at the same time.!?\r\n", ch );
    bug( "Do_sit: Character standing, but pos_obj != NULL. (%s)", ch->real_name( ) );
    return;
  }

  if( ( thing = one_thing( OBJ_DATA, ch, argument, "sit on", ch->array ) ) == NULL ) 
    return;
  ...
  /*
  ** DISABLED:  Checking for too many people sitting on an object.
  **      WHY:  There is no Obj_Data::character.
  int              i;
  char_data*     sch;
  for (i = 0, sch = obj->character; sch != NULL; i++, sch = sch->next_on_obj)
     ; 
  ...
  */
  ...
  if( ch->rider != NULL ) {
    send( ch, "You sit on %s, dismounting %s.\r\n", obj->pIndexData->Name( ), ch->rider );
    send( ch->rider, "Your mount sits on %s, dismounting you in the process.\r\n", obj );
    fsend_seen( ch, "%s sits on %s, dismounting %s in the process.", ch, obj, ch->rider );
    dismount( ch->rider );
  } else {  
...
void sleep( char_data *ch )
{
  ...
  if( ch->rider != NULL ) {
    send( ch->rider, "As %s drops to sleep, you quickly find yourself dismounted.\r\n", ch );
    fsend_seen( ch->rider, "As %s falls to sleep, %s quickly finds %sself dismounted.", ch, ch->rider, ch->rider->Him_Her( ) );
    dismount( ch->rider );
  }
...
void do_stand( char_data* ch, char* )
{
  ...
  if( ch->pos_obj != NULL ) {
    if ( ch->pos_obj->valid == OBJ_DATA ) {
      obj_data* chair = ch->pos_obj;
      chair->selected = 1;
      chair->shown = 1;
      switch( ch->position ) {
      case POS_SLEEPING:
        send( ch, "You wake up from your rest on %s.", chair );
        send_seen( ch, "%s wakes up from %s rest on %s.", ch, ch->Him_Her( ), chair );
        break;
      case POS_MEDITATING:
        send( ch, "You stop meditating on %s.", chair );
        send_seen( ch, "%s stops meditating on %s.", ch, chair );
        break;
      }
...
void do_wake( char_data* ch, char* argument )
{
  ...
  if( ch->position <= POS_MEDITATING ) {
    send( ch, "Perhaps you should stop %s before you try to wake others.\r\n", position_verb[ ch->position ] );
    return;
  }
