...
int max_language    = LANG_MAX - LANG_FIRST;
int max_location    = MAX_LOCATION;
int max_quest_flags = MAX_QUEST_FLAGS;

flag_data affect_flags = { "Affect", &aff_char_table[0].name, &aff_char_table[1].name, &MAX_ENTRY_AFF_CHAR };
flag_data permission_flags = { "Permission", &permission_name[1], &permission_name[2], &max_permission };
flag_data material_flags = { "Materials", &material_table[0].name, &material_table[1].name, &MAX_ENTRY_METAL };
flag_data align_flags = { "Alignment", &alignment_table[0].name, &alignment_table[1].name, &MAX_ENTRY_ALIGNMENT };
flag_data abv_align_flags = { "Alignment", &alignment_table[0].abbrev, &alignment_table[1].abbrev, &MAX_ENTRY_ALIGNMENT };
flag_data lang_flags = { "Languages", &skill_table[LANG_FIRST].name, &skill_table[LANG_FIRST+1].name, &max_language };
flag_data location_flags = { "Location", &location_name[0], &location_name[1], &max_location };
flag_data race_flags = { "Races", &race_table[0].name, &race_table[1].name, &MAX_ENTRY_PLYR_RACE };
flag_data clss_flags = { "Classes", &clss_table[0].name, &clss_table[1].name, &MAX_ENTRY_CLSS };
flag_data terrain_flags = { "Terrain", &terrain_table[0].entry_name, &terrain_table[1].entry_name, &MAX_ENTRY_TERRAIN };
flag_data quest_flags = { "Quests", &quest_flag_name[0], &quest_flag_name[1], &max_quest_flags };
...
const char *flag_data :: set( char_data* ch, char* argument, int* bit )
{
  if( *argument == '\0' ) {
    display( ch, bit );
    return NULL;
  } else
    return set_flags( name1, name2, bit, *max, NULL, ch, argument, FALSE, TRUE );
}
  ...
  bool exact, bool fail_msg )
{
  ...
  for( int i = 0; i < max; i++ ) {
    if( match_all_args( argument, *(name1+i*(name2-name1)) ) ) {
      if( can_edit != NULL ) {
        send( ch, can_edit );
        return can_edit;
      } 
...
void set_bool( char_data* ch, char* argument, const char* text, int& flag )
{
...
bool toggle( char_data* ch, char* arg, const char* text, int* bit, int flag )
{
  bool already;
...
bool get_flags( char_data* ch, char*& argument, int* bit, const char* flags, const char* function )
{
  int     i;
