...
int str_cmp(const char *arg1, const char *arg2)
{
  ...
  for (i = 0; arg1[i] || arg2[i]; i++)
    if ((chk = tolower(arg1[i]) - tolower(arg2[i])) != 0)
...
int _strcmp( const char* astr, const char* bstr )
{
  if( astr == bstr )
    return 0;
...
bool matches( char*& argument, const char* word )
{
  return matches( (const char*&) argument, word );
}
...
bool exact_match( char*& argument, const char* word )
{
  return exact_match( (const char*&) argument, word );
}


bool exact_match_all_args( const char*& argument, const char* word )
{
  const char* str = argument;
...
bool exact_match_all_args( char*& argument, const char* word )
{
  return exact_match_all_args( (const char*&) argument, word );
}


bool match_all_args( const char*& argument, const char* word )
{
  // try exact match first
  if( exact_match_all_args( argument, word ) )
    return true;
...
bool match_all_args( char*& argument, const char* word )
{
  return match_all_args( (const char*&) argument, word );
}


bool number_arg( char*& argument, int& i )
{
  char*  str = argument;
  int      j = 0;
  bool minus = FALSE;
...
char* one_condition( char *argument, char *cond )
{
  if( *argument == '\0' )
    *cond = '\0';
    ...
    for( ; *argument != '\r' && *argument != '\n' && *argument != '\0' && *argument != '&'; argument++, cond++ )
      *cond = *argument;
...
bool contains_word( char*& argument, const char* word, char* arg )
{
  char*  s1  = argument;
  char*  s2;
...
bool two_argument( char*& argument, const char* word, char* arg )
{
  int i;

  if( ( i = count_args( argument ) ) < 2 )
    return FALSE;
  ...
  return( contains_word( argument, word, arg ) && *argument != '\0' && *arg != '\0' );
}
...
char* capitalize_words( const char* argument )
{
  ...
  *tmp = toupper( *argument );
  ...
  for( i = 1; argument[i] != '\0'; i++ ) 
    tmp[i] = ( argument[i-1] == ' ' ? toupper( argument[i] ) : argument[i] );
