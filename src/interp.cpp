...
extern bool confused_char ( char_data* ch );
...
{
  if( ch->pcdata != NULL || ch->leader == NULL || !is_set( &ch->status, STAT_ORDERED ) )
    return FALSE;
...
{
  if( ch->pcdata == NULL || ch->species == NULL || ch->shdata->level >= LEVEL_BUILDER )
    return FALSE;
...
{
  if( ch->pcdata == NULL || ch->shifted == NULL || is_apprentice( ch ) || is_set( ch->shifted->act_flags, ACT_CAN_CAST ) )
    return FALSE;

  send( ch, "You can't do that while shifted in form.\r\n" );
...
void interpret( char_data* ch, char* argument )
{
  ...
  if (!ch || !ch->Is_Valid()) {
    roach("interpret(): NULL character!");
    return;
  }
  ...
  if( ch->pcdata != NULL && is_set( ch->pcdata->pfile->flags, PLR_FREEZE ) && !is_god( ch ) ) {
  ...
  if( !found ) {
    if( !check_social( ch, command, arg ) /* && !speed_walking( ch, command ) */ )
  ...
  if( command_table[cmd].queue && ch->active.time != -1 ) {
    if( ( time_till( &ch->active ) > 5 || entries( ch->cmd_queue ) != 0 ) && ch->pcdata != NULL && is_set( &ch->pcdata->message, MSG_QUEUE ) )
      send( ch, "Queued: %s\r\n", argument );
    push( ch->cmd_queue, argument, is_set( &ch->status, STAT_ORDERED ) );
    return;
  }

  if( command_table[cmd].position > POS_SLEEPING && is_set( ch->affected_by, AFF_PARALYSIS ) ) {
  ...
  if( command_table[cmd].position > POS_SLEEPING && 
    is_set( ch->affected_by, AFF_CONFUSED ) )
    if( confused_char( ch ) )
      return;

  if( is_avatar( ch ) || ( ch->pcdata != NULL && is_set( ch->pcdata->pfile->flags, PLR_LOGFILE ) ) )
    immortal_log( ch, command, arg );

  if( command_table[cmd].function == NULL ) {
    send( ch, "Command %s has no routine assigned to it?!\r\n", command_table[cmd].name );
    bug( "Interpret: command %s has no routine.", command_table[cmd].name );
    return;
  }
...
bool check_progs( char_data* ch, char* command, char* argument )
{
  ...
  // check inventory for the supplied oprog "command argument"
  for( i = 0; i < ch->contents.size; i++ ) {
    obj = object( ch->contents[i] );
    if( obj == NULL || obj->valid != OBJ_DATA )
      continue;
  ...
  // check equipment for the supplied oprog "command argument"
  for ( i = 0; i < ch->wearing.size; i++ ) {
    obj = object( ch->wearing[i] );
    if( obj == NULL || obj->valid != OBJ_DATA )
      continue;
  ...
  // check all objects in room for the supplied oprog "command argument"
  for ( i = 0; i < ch->array->size; i++ ) {
...
void do_commands( char_data* ch, char* argument )
{
  ...
  if( is_set( &flags, 0 ) ) {
    ...
    if( !has_permission( ch, pf ) ) {
      send( ch, "You cannot view commands you do not have permission for.\r\n" );
      return;
    } 
    for( i = 0, j = 0; i < MAX_ENTRY_COMMAND; i++ ) {
      if( is_set( command_table[i].level, pf ) ) {
        if( !found ) {
          page_title( ch, "Commands requiring %s Flag", permission_name[pf] );
          found = TRUE;
        }
        page( ch, "%15s%s", command_table[i].name, ++j%4 ? "" : "\r\n" );
      } 
    }
    if( !found ) 
      send( ch, "There are no commands which require the %s permission flag.\r\n", permission_name[pf+1] );
    else if( j%4 != 0 )
      page( ch, "\r\n" );        
    return;       
  }
  ...
  if( *argument == '\0' ) {
    ...
    for( i = 0, j = 0; i < MAX_ENTRY_CMD_CAT; i++ ) {
      if( trust >= cmd_cat_table[i].level ) {
        page( ch, "%15s%s", cmd_cat_table[i].name, ++j%4 ? "" : "\r\n" );
      }
    }
    page( ch, "\r\n%s", j%4 != 0 ? "\r\n" : "" );
  ...
  for( i = 0; i < MAX_ENTRY_CMD_CAT; i++ ) 
    if( trust >= cmd_cat_table[i].level && matches( argument, cmd_cat_table[i].name ) ) {
      page_title( ch, "%s Commands", cmd_cat_table[i].name );
      for( j = 0; j < MAX_ENTRY_COMMAND; j++ ) {
        if( command_table[j].category == i && has_permission( ch, command_table[j].level ) ) {
          page( ch, "%15s : %s\r\n", command_table[j].name, command_table[j].help );
        }
      }
      return;
    }
