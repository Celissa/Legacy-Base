...
void exec_range( char_data* ch, int range, path_func* function, char* arg1 )
{
  room_array       list;
  room_array       next;
  char_data*        rch;
        ...
        if( to_room->distance == MAX_INTEGER ) {
          ...
          for( int n = 0; n < to_room->contents; n++ )
            if( ( rch = character( to_room->contents[n] ) ) != NULL )
              ( *function )( rch, ch, arg1, dir_table[ exit->direction ].reverse, i+1 );
        }
      }
    }
    if( is_empty( next ) )
      break;
    m2swap( list.list, next.list );
    m2swap( list.size, next.size );
    clear( next );
  }
...
const char* whistle_msg = 
  "Your incessant whistling seems to have attracted a disgruntled mail daemon.\r\n"
...
void do_whistle( char_data* ch, char* )
{
  ...
  if( ( obj = has_whistle( ch ) ) != NULL ) {
    if( execute_use_trigger( ch, obj, OPROG_TRIGGER_USE ) )
      return;
    // need a suppress message on execution of a trigger, I suppose
    send( ch, "You blow %s producing a loud shriek.\r\n", obj );
    send( *ch->array, "%s blows %s producing a loud shriek.\r\n", ch, obj );
    range = obj->value[0];
  } else {
    if( ch->shdata->race == RACE_LIZARD ) {
      send( ch, "You try to whistle but the best you can do is hiss loudly.\r\n" );
...
void hear_whistle( char_data* victim, char_data* ch, char*, int dir, int )
{
  if( victim->position == POS_EXTRACTED || !can_hear( victim ) ) 
    return;

  send( victim, "You hear a whistle from somewhere %s.\r\n", dir_table[dir].where );        

  if( victim->pcdata != NULL || !is_set( &victim->status, STAT_PET ) || victim->leader != ch )
    return;
...
void respond_summon( char_data* victim, char_data* ch, char*, int, int )
{
  if( !ch->species )
    return;
  ...
  if( victim->pcdata != NULL || victim->position < POS_RESTING || !is_set( victim->species->act_flags, ACT_ASSIST_GROUP ) )
    return;

  if( ( nation == NATION_NONE || nation != victim->species->nation ) && ( group == GROUP_NONE || group != victim->species->group ) )
    return;
...
void plot( map_data map [61][161], int length, int width, int pos, char letter, char color, void* pointer, int& top, int& bottom )
{
  int l = pos%1024-512;
  int x = (pos/1024)%1024-512+width/2;
  int y = (pos/1024/1024)-512+length/2;

  if( l != 0 || x < 0 || x >= width || y < 0 || y >= length ) 
    return;

  top    = min( top, y );
  bottom = max( bottom, y );

  if( map[y][x].pointer != NULL && ( map[y][x].pointer != pointer || map[y][x].letter != letter ) ) {
    map[y][x].letter = '?';
    map[y][x].color = 'R';
  } else {
    map[y][x].letter = letter;
    map[y][x].color = color;
    map[y][x].pointer = pointer;
  }
}
...
void show_map( char_data* ch, int length, int width )
{
  ...
  map[length/2][width/2].letter = '*';
  map[length/2][width/2].color = 'Y';
  map[length/2][width/2].pointer = ch->in_room;
  ...
  *list1 += ch->in_room;
  ...
  for( ; !is_empty( *list1 ); ) {
    for( int i = 0; i < *list1; i++ ) {
      ...
      for( int j = 0; j < room->exits; j++ ) {
        ...
        plot( map, length, width, pos, is_set( &exit->exit_info, EX_ISDOOR ) ? '+' : sym_exit[ exit->direction ], 'c', (void*) 1, top, bottom );

        if( to_room->distance == 0 && ( pos = offset( pos, exit->direction ) ) != -1 ) {
          Exit_Data *up   = exit_direction( to_room, DIR_UP );
          Exit_Data *down = exit_direction( to_room, DIR_DOWN );

          char color = ( water_logged( to_room ) ? 'B' : 'C' );
          if( is_apprentice( ch ) )
            color = ( ch->in_room->area != to_room->area ? tolower( color ) : color );

          plot( map, length, width, pos, up ? ( down ? 'X' : '>' ) : ( down ? '<' : ( is_set( &to_room->room_flags, RFLAG_APPROVED ) ? 'o' : 'O' ) ), color, room, top, bottom );
  ...
  char tmp [ 3*MAX_STRING_LENGTH ], tmp2 [ 3*MAX_STRING_LENGTH ];

  for( int i = 0; i <= bottom - top; i++ ) { 
    tmp[ 0 ]= '\0';

    for( int j = 0; j <= width; j++ ) {
      if( map[ i + top ][ j ].pointer == NULL )
        strcat( tmp, " " );
      else
        sprintf( tmp + strlen( tmp ), "@%c%c@n", map[ i + top ][ j ].color, map[ i + top ][ j ].letter );
    }

    if( i < 11 )
      sprintf( tmp + strlen( tmp ), "%5s%s", "", key[ i ] );
    strcat( tmp, "\r\n" );

    convert_to_ansi( ch, tmp, tmp2, sizeof( tmp2 ) );
    send( ch, tmp2 );
  }
}
