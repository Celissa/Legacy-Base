...
void send_to_area( const char* string, area_data* area, int sense )
{
  char_data *rch;
  room_data *room;
  ...
  for( room = area->room_first; room != NULL; room = room->next )   
    for( int i = 0; i < room->contents; i++ )    
      if( ( rch = character( room->contents[i] ) ) != NULL && rch->link != NULL ) {
        act_print( buf, string, NULL, NULL, NULL, NULL, NULL, NULL, rch );
        convert_to_ansi( rch, buf, tmp, sizeof( tmp ) );
        act( rch, tmp, NULL, NULL, NULL, NULL, sense );
      }
