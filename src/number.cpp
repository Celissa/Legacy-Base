...
double   fevaluate    ( char*, bool&, double = 0, double = 0 );
double   fevaluate    ( char*, bool&, double, double, int, int ); 
...
const char* number_word( int i, char_data* ch )
{
  ...
  if( i > 99 ) {
    sprintf( tmp, "%s hundred and ", number_name[i/100] );
    if( i%100 < 20 )
...
int evaluate( char* string, bool& error, int v1, int v2, int left, int right ) 
{
  char  priority  = '4';
  int   division  = -1;
  int       a, b;
...
const char* spec_tostring( double x, int dp = 4)
{
  char* tmp = static_string( );

/*
  int y = (int) ( x * pow( 10, dp + 1 ) );
...
void do_calculate( char_data* ch, char* argument )
{
  ...
  send( ch, "%s = %s\r\n", argument, spec_tostring( x ) );

  return;
}
 

double fevaluate( char* string, bool& error, double v1, double v2 )
{
  error = FALSE;
...
double fevaluate( char* string, bool& error, double v1, double v2, int left, int right ) 
{
  char          priority      = '4';
  int           division      = -1;
  double            a, b;
...
#define NVAR 2
  ...
  char*   var_name   [ NVAR ]  = { "pi", "e" };
  double  var_value  [ NVAR ]  = { M_PI, M_E };
