...
//  room_data*      room;
  ...
  wand = one_object( ch, arg, "zap", &ch->contents, &ch->wearing );
  if( wand == NULL ) {
    send( ch, "You do not have that wand.\r\n" );
    return;
  }

  if( ch->Wearing( WEAR_HELD_R, LAYER_BASE ) != wand
  && ch->Wearing( WEAR_HELD_L, LAYER_BASE ) != wand ) {
  ...
  if( ( spell = wand->pIndexData->value[0] ) < 0  || spell >= SPELL_COUNT ) {
  ...
  victim  = character( cast->target );
  obj     = object( cast->target );
//  room    = cast->room;

  delete cast;

  if( execute_use_trigger( ch, wand, OPROG_TRIGGER_USE ) )
    return;
  ...
  if( spell_table[ spell ].type == STYPE_OFFENSIVE && victim != NULL ) {
  ...
  ( *spell_table[ spell ].function )( ch, victim, obj, level, duration ); 
//  ( *spell_table[ spell ].function )( ch, victim, (room == NULL ? (void *) obj : (void *) room), level, duration ); 
...
void do_quaff( char_data* ch, char* argument )
{
  ...
  if( ( spell = obj->pIndexData->value[0] ) < 0 || spell >= SPELL_COUNT ) {
  ...
  ( *spell_table[ spell ].function )( ch, ch, NULL, level, duration );
...
void do_recite( char_data* ch, char* argument )
{
  ...
  if( !is_set( ch->pcdata->pfile->flags, PLR_HOLYLIGHT ) && ( is_set( ch->affected_by, AFF_BLIND ) || ch->in_room->is_dark( ) ) ) {
    send( ch, "It is too dark for you to read any writing.\r\n" );
    return;
  }

  if( execute_use_trigger( ch, scroll, OPROG_TRIGGER_USE ) )
    return;

  if( ( spell = scroll->pIndexData->value[0] ) < 0 || spell >= SPELL_COUNT ) {
  ...
  if( spell == SPELL_RECALL-SPELL_FIRST 
    ...
    send( ch, "You attempt to pronounce the words on the scroll but absolutely nothing\r\nhappens.\r\n" );
    return;
  }
  ...
  ( *spell_table[ spell ].function )( ch, victim, target, level, duration );
