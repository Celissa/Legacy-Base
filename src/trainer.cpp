...
void set_trainer( mob_data* mob, room_data* room )
{
  ...
  for( trainer = trainer_list; trainer != NULL; trainer = trainer->next ) 
    if( trainer->room == room && trainer->mob == NULL && trainer->trainer == mob->species->vnum ) {
  ...
  for( i = 0; i < MAX_SKILL; i++ ) {
    ...
    for( trainer = trainer_list; trainer != NULL; trainer = trainer->next ) {
      ...
      if( is_set( trainer->skill, i ) ) {
        page( ch, "  [%4d] %-30s [%5d] %s\r\n", trainer->trainer,
          species == NULL ? "no one?" : species->Name( ),
...
void do_tnofind( char_data* ch, char* argument )
{
  bool               found;
  trainer_data*    trainer;

  if( *argument != '\0' ) { 
    send( ch, "Syntax: tnofind\r\n" );
    return;
  }
  ...
  for( int i = 0; i < MAX_SKILL; i++ ) {
    found = false;
    for( trainer = trainer_list; trainer != NULL && !found; trainer = trainer->next ) {
      if( is_set( trainer->skill, i ) ) 
        found = true;
    }
    if( !found && strncmp( skill_table[i].name, "uskill", 5 ) &&
      strncmp( skill_table[i].name, "ulang", 4 ) &&
      strncmp( skill_table[i].name, "uspell", 5 ) &&
      strncmp( skill_table[i].name, "utrade", 5 ) &&
      strncmp( skill_table[i].name, "uweapon", 6 ) )
      page( ch, "  No trainers found for %s.\r\n", skill_table[i].name );
    }
  ...
  if( !is_number( argument ) ) {
    for( i = 0; i < MAX_SKILL; i++ ) {
      if( fmatches( argument, skill_table[i].name ) ) {
        ...
        send( ch, "%s can %s train %s.\r\n", victim->Seen_Name( ch ), is_set( trainer->skill, i ) ? "now" : "no longer", skill_table[i].name );
        return;
      }
    } 
  } else if( ( i = atoi( argument ) ) >= 0 && i < MAX_SKILL ) {
    switch_bit( trainer->skill, i );
    send( ch, "%s can %s train %s.\r\n", victim->Seen_Name( ch ), is_set( trainer->skill, i ) ? "now" : "no longer", skill_table[i].name );
    return;
  } else {
    send( ch, "A skill number must be between 0 and %i.\r\n", MAX_SKILL );
  }
...
void save_trainers( )
{
  ...
  rename_file( AREA_DIR, TRAINER_FILE, PREV_DIR, TRAINER_FILE );

  if( ( fp = open_file( TRAINER_FILE, "wb" ) ) == NULL ) 
    return;
