...
#define COLOR_IMMORTAL              8
#define COLOR_AVATAR                9
...
#define MXP_LOCK         "\x1B[7z"
#define MXP_SECURE       "\x1B[4z"
#define MXP_OPEN         "\x1B[5z"
#define MXP_PERM_SECURE  "\x1B[6z"

#define MXP_RESET        "\x1B[3z" MXP_LOCK 
...
void          convert_to_ansi  ( char_data*, const char*, selt_string* );  
...
inline void save_cursor      ( char_data* ch )  { send( ch, "\x1B\x37" ); }
inline void restore_cursor   ( char_data* ch )  { send( ch, "\x1B\x38" ); }
inline void cursor_on        ( char_data* ch )  { send( ch, "\x1B[?25h" ); }
inline void cursor_off       ( char_data* ch )  { send( ch, "\x1B[?25l" ); }
inline void lock_keyboard    ( char_data* ch )  { send( ch, "\x1B[2l" ); }
inline void unlock_keyboard  ( char_data* ch )  { send( ch, "\x1B[2h" ); }


inline void move_cursor( char_data* ch, int line, int column )
{
  send( ch, "\x1B[%d;%dH", line, column );
}
  

inline void scroll_region( char_data* ch, int top, int bottom )
{
  send( ch, "\x1B[%d;%dr", top, bottom );
}
