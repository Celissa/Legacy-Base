...
bool can_carry(char_data *ch, thing_array *list, bool msg)
{
  int number = 0;
  int weight = 0;
  ...
  if( number+ch->contents.number > ch->can_carry_n( ) ) {
    if( msg )
      send(ch, "[ too many items ]\r\n" );
    return FALSE;
  }

  if (weight > ch->Capacity()) {
    if (msg)
      send(ch, "[ too heavy ]\r\n" );
    return FALSE;
  }
...
{
  if (obj->Number() + ch->contents.number > ch->can_carry_n()) {
    if (msg)
...
int Obj_Data :: Weight( int i )
{
  ...
  if( pIndexData->item_type == ITEM_CONTAINER && is_set( &value[1], CONT_HOLDING ) ) 
    sum /= 2;

  if( pIndexData->item_type == ITEM_DRINK_CON && value[1] > 0 )
    sum += int( 2.2*value[1] );
...
{
  for( int i = MAT_BRONZE; i <= MAT_LIRIDIUM; i++ )
...
{
  if( ( pIndexData->item_type != ITEM_WEAPON && pIndexData->item_type != ITEM_ARMOR )
