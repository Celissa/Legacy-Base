...
bool in_bank( char_data* ch )
{
  ...
  if(!ch || !ch->array || ( room = Room( ch->array->where ) ) == NULL
    ||( !is_set( &room->room_flags, RFLAG_BANK ) && !is_apprentice( ch ) ) ) {
...
void do_balance( char_data* ch, char* argument )
{
  ...
  if( *argument != '\0' && is_apprentice( ch ) ) {
    if( ( victim = one_player( ch, argument, "balance", (thing_array*) &player_list ) ) == NULL )
      return;
  }
  ...
  page_underlined( ch, "Items in Storage                                         Weight\r\n" );
  ...
  for( int i = 0; i < pc->locker; i++ ) {
    thing = pc->locker[i];
    if( thing->shown > 0 ) {
      page( ch, "%-55s %7.2f\r\n", thing->Seen_Name( ch, thing->shown, TRUE ), .01*thing->temp );
    }
  }
...
void do_deposit( char_data* ch, char* argument )
{
  ...
  if( ch->shifted != NULL ) {
    send( ch, "The bankers refuse to deal with animals.\r\n" );
    return;
  }
  ...
  if( ( array = several_things( OBJ_DATA, ch, argument, "deposit", &ch->contents ) ) == NULL )
    return;
...
const char* auction_msg = "That would take your bank balance below what\
 you have bid on the\r\nauction block and if you did that the delivery daemons would rip you apart\r\nand play catch with the pieces.\r\n";
...
void do_withdraw( char_data* ch, char* argument )
{
  ...
  if( ( array = several_things( OBJ_DATA, ch, argument, "withdraw", &pc->locker ) ) == NULL ) 
    return;
