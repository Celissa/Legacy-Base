...
void do_mount( char_data* ch, char* argument )
{
  ...
  if( IS_NPC( ch ) || ch->get_skill( SKILL_RIDING ) == UNLEARNT ) {
  ...
  if( ( victim = one_character( ch, argument, "mount", ch->array ) ) == NULL )
    return;
  ...
  if( victim->leader != ch || !is_set( &victim->status, STAT_PET ) || !is_set( victim->species->act_flags, ACT_MOUNT ) ) {
...
void check_mount( char_data* ch )
{
  ...
  if( number_range( -10, 2*MAX_SKILL_LEVEL ) < ch->get_skill( SKILL_RIDING )+ch->get_skill( SKILL_MOUNTED_FIGHTING ) )
    return;

  send( ch, "You are thrown from your mount.\r\n" );
  fsend_seen( ch, "%s is thrown from %s which %s was riding.", ch, ch->mount, ch->He_She( ) );
