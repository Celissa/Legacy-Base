...
void do_energize( char_data* ch, char* argument )
{
  ...
  if( ch->get_skill( SKILL_TRANSFER_ENERGY ) == UNLEARNT ) {
  ...
  if( *argument == '\0' ) {
    ...
    if( ( victim = one_character( ch, argument, "energize", ch->array ) ) == NULL ) {
  ...
  if( victim->fighting != NULL ) {
    send( ch, "You can't transfer energy to someone who is fighting!\r\n" );
    return;
  }
  ...
  cure = ( heal*( ch->get_skill( SKILL_TRANSFER_ENERGY )
     + ch->get_skill( SKILL_ENERGY_SHIFT ) ) )/(2*MAX_SKILL_LEVEL ); 

  if( ch->mana-heal > 0 ) {
    victim->mana = victim->mana + (int) cure;
  ...
  if( number_range( 0, 1 ) == 0 )
    ch->improve_skill( SKILL_ENERGY_SHIFT );
  else
