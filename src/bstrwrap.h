...
extern struct CBStringException CBStringExceptionStatic;
#define bstringThrow(er) {\
	CBStringExceptionStatic.msg = ("CBString::" er);\
	throw (&CBStringExceptionStatic);\
}
#else
#define bstringThrow(er)
#endif
...
struct CBString : public tagbstring {
	...
	void vformata (const char * fmt, va_list arglist);
	void format (const char * fmt, ...);
	void formata (const char * fmt, ...);
	void fill (int length, unsigned char fill = ' ');
	void repeat (int count);
	void ltrim (const CBString& b = CBString (" \t\v\f\r\n", sizeof (" \t\v\f\r\n")));
	void rtrim (const CBString& b = CBString (" \t\v\f\r\n", sizeof (" \t\v\f\r\n")));
	void toupper ();
	void tolower ();
