...
#define PERM_OBJ_APPROVAL         37
#define PERM_MOB_APPROVAL         38
#define PERM_ROOM_APPROVAL        39
#define MAX_PERMISSION            40
...
inline bool is_avatar     ( char_data* ch )  { return get_trust( ch ) >= LEVEL_AVATAR; }
inline bool is_apprentice ( char_data* ch )  { return get_trust( ch ) >= LEVEL_APPRENTICE; }
inline bool is_builder    ( char_data* ch )  { return get_trust( ch ) >= LEVEL_BUILDER; }
inline bool is_immortal   ( char_data* ch )  { return get_trust( ch ) >= LEVEL_IMMORTAL; }
inline bool is_spirit     ( char_data* ch )  { return get_trust( ch ) >= LEVEL_SPIRIT; }
inline bool is_angel      ( char_data* ch )  { return get_trust( ch ) >= LEVEL_ANGEL; }
inline bool is_demigod    ( char_data* ch )  { return get_trust( ch ) >= LEVEL_DEMIGOD; }
inline bool is_god        ( char_data* ch )  { return get_trust( ch ) == LEVEL_GOD; }

void shutdown_sequence( char* = NULL );
