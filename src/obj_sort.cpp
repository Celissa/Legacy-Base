...
thing_data* cursed( thing_data* thing, char_data* ch, thing_data* )
{ 
  obj_data* obj = object( thing );
...
thing_data* no_junk( thing_data* thing, char_data*, thing_data* )
{ 
  obj_data* obj = object( thing );
  ...
  return (obj->junkable() ? obj : NULL);
}

thing_data* no_show( thing_data* thing, char_data* ch, thing_data* )
{
  obj_data* obj;
  ...
  int n, thing_array* subset, thing_func** func )
{
  ...
  for( int i = 0; i < array; i++ ) {
    thing = array[i];
    for( int j = 0; ; j++ ) {
      unsorted = ( func[j] == NULL ? thing : ( *func[j] )( thing, ch, container ) );
      if( j == n-1 ) {
        // if last subset, add unsorted to array also
        subset[j] += unsorted;
        break;
      }
      if( unsorted != thing ) {
        // if unsorted is different from what we passed, add (all or the remainder of)
        // 'thing' to the unsorted list
        subset[j] += thing;
      }
      if( ( thing = unsorted ) == NULL ) {
        // if function returned NULL, go to next item
        break;
      }
    }
  }
  ...
  thing_data* container, const char* msg2, const char* msg3 )
{
  ...
  if( ( thing = one_shown( *array ) ) != NULL ) {
    ...
    if( container == NULL ) {
      ...
      fsend( ch, "You %s %s %s %s.", msg1, thing, msg2, container );
      }
    else {
...
void page_publ( char_data* ch, thing_array* array, const char* msg1, thing_data* container, const char* msg2, const char* msg3, bool buy )
{
  char_data*  rch;
  thing_data*   thing;
  char              buf [ FOUR_LINES ];
  ...
  if( ( thing = one_shown( *array ) ) != NULL ) { 
    ...
    if( !strcmp( msg3, "for" ) ) {
      int cost = thing->temp;
      cost += buy ? add_tax( ch->in_room, thing->temp, false ) : 0;
      fsend( ch, "You %s %s %s %s for %d cp.",
        msg1, thing, msg2, container, cost );
    }
    else if( container != NULL ) 
      fsend( ch, "You %s %s %s %s.",
        msg1, thing, msg2, container );
    else 
      fsend( ch, "You %s %s%s%s%s%s.", msg1, thing, *msg2 != '\0' ? " " : "", msg2, *msg3 != '\0' ? " " : "", msg3 );
  } else {
    if( !first_list )
      send( ch, "\r\n" );
    if( !strcmp( msg3, "for" ) )
      ...
      send( ch, "You %s%s%s%s%s:\r\n", msg1, *msg2 != '\0' ? " " : "", msg2, *msg3 != '\0' ? " " : "", msg3 );
    for( int i = 0; i < *array; i++ ) {
      thing = array->list[i];
      if( thing->shown > 0 )  
        if( !strcmp( msg3, "for" ) )
  ...
  for( int i = 0; i < *ch->array; i++ ) { 
    ...
    if( ( thing = one_shown( *array ) ) != NULL ) { 
      if( !strcmp( msg3, "for" ) ) 
        fsend( rch, "%s %ss %s %s %s.", ch, msg1, thing, msg2, container );
      else if( container != NULL ) 
        fsend( rch, "%s %ss %s %s %s.", ch, msg1, thing, msg2, container == rch ? "you" : container->Name( rch ) );
      else {
        sprintf( buf, "%s %ss %s%s%s%s%s.", "%s", strcmp( msg1, "empty" ) ?  msg1 : "emptie" , "%s", *msg2 != '\0' ? " " : "", msg2, *msg3 != '\0' ? " " : "", msg3 );
        fsend( rch, buf, ch, thing );
      }
//        fsend( rch, "%s %ss %s%s%s%s%s.", ch, strcmp( msg1, "empty" ) ?  msg1 : "emptie" , thing, *msg2 != '\0' ? " " : "", msg2, *msg3 != '\0' ? " " : "", msg3 );
      continue;
    }

    if( !is_set( &rch->pcdata->message, MSG_MULTIPLE_ITEMS ) ) {
      if( !strcmp( msg3, "for" ) )
        fsend( rch, "%s %ss several items %s %s.", ch, msg1, msg2, container );
      else if( container != NULL ) 
        fsend( rch, "%s %ss several items %s %s.", ch, msg1, msg2, container == rch ? "you" : container->Name( rch ) );
      else  
    ...
    if( !strcmp( msg3, "for" ) )
      ...
      send( rch, "%s %ss %s %s:\r\n", ch, msg1, msg2, container == rch ? "you" : container->Name( rch ) );
    else
      send( rch, "%s %ss%s%s%s%s:\r\n", ch, msg1, *msg2 != '\0' ? " " : "", msg2, *msg3 != '\0' ? " " : "", msg3 );
...
void send_priv( char_data* ch, thing_array* array, const char* msg1, thing_data* container )
{
  thing_data* thing;
...
void send_publ( char_data* ch, thing_array* array, const char* msg1, const char* msg2 )
{
  char_data*     rch;
  thing_data*  thing;
...
//  rehash( ch, *array );
  ...
  if( ( thing = one_shown( *array ) ) != NULL ) {
    ...
    if( obj->pIndexData->item_type == ITEM_LIGHT || obj->pIndexData->item_type == ITEM_LIGHT_PERM ) 
      page( ch, "You %s %s which flares up, lighting the darkness.\r\n", wear_verb[ thing->position ], thing );
    else
      page( ch, "You %s %s.\r\n", wear_verb[ thing->position ], thing );
  } else {
    if( !first_list )
    ...
    for( int i = 0; i < *array; i++ ) {
      ...
      if( thing->shown > 0 ) {
        ...
        page( ch, "  %s %s.\r\n", wear_verb[ thing->position ], thing );
        if( obj->pIndexData->item_type == ITEM_LIGHT || obj->pIndexData->item_type == ITEM_LIGHT_PERM ) {
          page( ch, "  ... %s flares up, lighting the darnkess.\r\n", thing );
        }
      }
    }
  }
