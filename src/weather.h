...
class Time_Data : public timeval
{
 public:
  ...
  bool operator<( const time_data& t ) {
    return( tv_sec == t.tv_sec ? tv_usec < t.tv_usec : tv_sec < t.tv_sec );
  }    
...
void               sprintf_datetime ( char*, const time_t );
char*              ltime            ( const time_t& );
void               time_update      ( void );
const time_data&   stop_clock       ( struct timeval& );
int                time_arg         ( char*&, char_data*, bool = true, bool = false );
int                week_day         ( void );
void               init_time        ( );
...
#define MONTHS_IN_GAME_YEAR    8
#define DAYS_IN_GAME_WEEK      7


extern const char*  game_month_name    [ MONTHS_IN_GAME_YEAR ];
extern const char*  game_day_name      [ DAYS_IN_GAME_WEEK ];
extern const int    days_in_game_month [ MONTHS_IN_GAME_YEAR ];
