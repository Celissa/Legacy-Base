...
const char* mprog_trigger[ MAX_MPROG_TRIGGER ] = { "entering",
  ...
  "skin", "timer", "attack", "order", "type.item", "abandon", "practice",
  "random", "tame" };
...
void mprog_data :: display( char_data* ch )
{
  ...
  if( mprog == NULL ) {
    send( ch, "Mob %-11d %-20s %s\r\n", species->vnum, "Fight Code", species->Name() );
  } else {
    send( ch, "Mob %-11d Mprog %-8d %s\r\n", species->vnum, i, species->Name() );
  }
...
void do_mpdata( char_data* ch, char* argument )
{
  ...
  if( wizard->mpdata_edit != NULL ) {
    ...
    wizard->mpdata_edit->text = edit_string( ch, argument, wizard->mpdata_edit->text, MEM_EXTRA );
  } else {
    edit_extra( mprog->data, wizard, offset( &wizard->mpdata_edit, wizard ), argument, "mprog" );
  }
...
void do_mpstat( char_data* ch, char* )
{
  ...
  page( ch, "%10s : %s\r\n", "Trigger", mprog_trigger[ mprog->trigger ] );
  page( ch, "%10s : %d\r\n", "Value", mprog->value );
  page( ch, "%10s : %s\r\n\r\n", "String", mprog->string );
...
void do_mpset( char_data* ch, char *argument )
{
  ...
  if( matches( argument, "trigger" ) ) {
    set_type( ch, argument, mprog->trigger, "Trigger", "mprog", MAX_MPROG_TRIGGER, mprog_trigger );
    return;
    }
  ...
  if( matches( argument, "string" ) ) {
    if( matches( argument, "delete" ) ) {
      if( mprog->string != NULL ) {
        if( mprog->string[0] != '\0' ) {
          free_string( mprog->string, MEM_MPROG );
        }
        mprog->string = empty_string;
        send( ch, "String deleted.\r\n" );
      }
    }
    else {
      set_string( ch, argument, mprog->string, "string", MEM_MPROG );
    }
    return;
  }
...
void do_ask( char_data* ch, char* argument )
{
  ...
  if( ( victim = one_character( ch, arg, "ask", ch->array ) ) == NULL )
    return;
  ...
  for( mprog = victim->species->mprog; mprog != NULL; mprog = mprog->next )
    if( mprog->trigger == MPROG_TRIGGER_ASKING && is_name( argument, mprog->string ) ) {
      var_ch  = ch;
      var_mob = victim;
      var_room = ch->in_room;
      var_arg = argument;
  ...
  for( mprog = victim->species->mprog; mprog != NULL; mprog = mprog->next )
    if( mprog->trigger == MPROG_TRIGGER_ASKING && is_name( "default", mprog->string ) ) {
      var_ch  = ch;
      var_mob = victim;
      var_room = ch->in_room;
      var_arg = argument;
  ...
  process_ic_tell( victim, ch, "I know nothing about that.", "say" );
