...
void  copy_extras    ( extra_array&, extra_array& );
...
inline bool can_see_who( char_data* ch, char_data* victim )
{
  ...
  if( ( imm = wizard( victim ) ) != NULL && is_set( victim->pcdata->pfile->flags, PLR_WIZINVIS ) && ch->shdata->level < imm->wizinvis )
    return FALSE;
