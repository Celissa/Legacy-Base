...
void do_throw( char_data* ch, char* argument )
{
  ...
  if( ( obj = one_object( ch, arg, "throw", &ch->contents ) ) == NULL ) {
  ...
  if( ( victim = one_character( ch, argument, "throw", ch->array ) ) == NULL) {
  ...
  if( obj->pIndexData->item_type != ITEM_DRINK_CON && obj->pIndexData->item_type != ITEM_POTION ) {
    send( ch, "That is not something you can throw with any result yet.\r\n" ); 
    return;
  }
...
bool throw_liquid( char_data* ch, char_data* victim, obj_data* obj )
{
  ...
  if( roll < 0 ) {
    throw_message( ch, victim, obj, "%s throw%s %s at %s, but it misses %s completely!", victim->Him_Her( ) );
    return FALSE;
  }
  ...
  if( roll < 0 ) {
    throw_message( ch, victim, obj, "%s throw%s %s at %s, but %s adriotly dodge%s it!", victim->He_She( ) );
    return FALSE;
  }

  throw_message( ch, victim, obj, "%s throw%s %s at %s, splashing the contents all over %s!", victim->Him_Her( ) );
...
void throw_potion( char_data* ch, char_data* victim, obj_data* obj )
{
  ...
  if( throw_liquid( ch, victim, obj ) ) {
    ...
    if( obj->value[0] < 0 || obj->value[0] >= SPELL_COUNT ) {
...
void throw_drink_con( char_data* ch, char_data* victim, obj_data* obj )
{
  ...
  if( obj->value[1] == 0 ) {
    obj->value[1] = -2;
    send( ch, "%s is empty - you need to go fill it with soup first.\r\n", obj );
    obj->value[1] = 0;
    return;
  }

  if( liquid >= table_max[ TABLE_LIQUID ] || liquid < 0 ) {
    bug( "Throw_Drink_Con: bad liquid number. (Obj# %d, Liq# %d)", obj->pIndexData->vnum, liquid );
  ...
  if( throw_liquid( ch, victim, obj ) && spell != -1 ) {
    ...
    ( *spell_table[spell-SPELL_FIRST].function )( ch, victim, NULL, 10, 5 ); 
  }
...
void throw_message( char_data* ch, char_data* victim, obj_data* obj, const char* text, const char* word )
{
