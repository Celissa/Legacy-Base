...
void do_request( char_data* ch, char* argument )
{
  ...
  if( *argument == '\0' ) 
  {
    if ( ch->pcdata != NULL && ch->pcdata->trust < LEVEL_AVATAR )
    {
  ...
  if( matches( argument, "avatar" ) ) {
    ...
    if( *argument == '\0'
      && is_set( ch->pcdata->pfile->flags, PLR_APPROVED ) ) {
      send( ch, "Since you are already approved you must specify a reason.\r\n" );
      return;
      }
    request_app += new request_data( ch, argument );
    send( ch, "Request for an avatar registered.\r\n" );
    if( *argument == '\0' ) 
      sprintf( tmp = static_string( ), "%s requesting an avatar for approval.", ch->Name( ) );
    else
      sprintf( tmp = static_string( ), "%s requesting an avatar with reason '%s'.", ch->Name( ), argument );
    
    info( empty_string, LEVEL_AVATAR, tmp, IFLAG_REQUESTS, 2, ch );
    return;
  }
  ...
  if( matches( argument, "immortal" ) ) {
    ...
    sprintf( tmp = static_string( ), "%s requesting an immortal with reason '%s'.", ch->Name( ), argument );
    info( empty_string, LEVEL_APPRENTICE, tmp, IFLAG_REQUESTS, 1, ch );
    return;
  }
...
void do_approve( char_data* ch, char* argument )
{
  ...
  if( is_set( &flags, 2 ) ) {
    ...
    if( player->pcdata->trust >= LEVEL_AVATAR ) { // || ( player->shdata->level > 10 && ch->pcdata->trust == LEVEL_AVATAR ) ) {
      send( ch, "You don't have permission to edit their description.\r\n" );
      return;
    }

    send( ch, "You are now editing %s.\r\n", player->descr->name );
    send( player, "%s is now editing your appearance.\r\n", ch->descr->name );
  ...
  sprintf( tmp, "Approved %s.", player->real_name( ) );
  player_log( ch, tmp );
